...
ScoredOuputEntries DecoderState::decode()
{
    ...
    for (size_t i = 0; i < options.beam_width && i < prefixes_copy.size(); ++i) {
        ...
        if (ext_scorer != nullptr
            ...
            prefixes_copy[i]->get_path_vec(tokens, timesteps);
            auto prefix_length = tokens.size();
            auto words = ext_scorer->split_labels(tokens);
            // remove word insert
...
ScoredOuputEntries ctc_beam_search_decoder(const std::vector<std::vector<double>>& probs_seq,
                              ...
                              HotwordScorer* hotword_scorer)
{
    ...
    std::vector<std::future<ScoredOuputEntries>> res;
...
BatchResults ctc_beam_search_decoder_batch_with_states(
